            ...
            <tbody>
             ...
             <tbody>
			    ...
			    @foreach($images as $image)
			     <td><a href="{{ URL::to_asset('img/uploads/'.$image->id.'/thumb-'.$image->name) }}">
                     <img src="{{ URL::to_asset('img/uploads/'.$image->id.'/thumb-'.$image->name) }}" alt="" /></a>
                     {{ HTML::link_to_action('admin.groups@delete_photo', 'Fotoğraf Kaldır',array($group->id),array('class'=>'button radius')) }}</td>
			    @endforeach
		        </tr>
            </tbody>
	    </table>
