...
<h3>Grup <small> ve oyunları</small> Hakkında</h3>
  ...
  @foreach($shows as $show)
		<tr>
			<td><a href="<?php echo URL::to('show/'.$show->id); ?>">{{$show->name}}</a></td>
			<td><a href="<?php echo URL::to('show/'.$show->id); ?>">{{$show->age}}</a></td>
			<td><a href="<?php echo URL::to('show/'.$show->id); ?>">{{$show->language}}</a></td>
...
<div class="button-bar ">
 <ul class="button-group  radius">
			...
			<li>{{HTML::link_to_action('admin.groups@index_photo', 'Fotoğrafları Düzenle',array($group->id),array('class'=>'button radius'))}}</li>
